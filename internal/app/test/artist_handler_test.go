...
func TestGetAllArtists(t *testing.T) {
	var mockArtists = make([]domain.ArtistDataTransfer, 10)
	...
	mockAlbumUseCase := new(mocks.AlbumUseCaseInterface)
	mockArtistUseCase := new(mocks.ArtistUseCaseInterface)

	mockArtistUseCase.On("GetAll", mockAlbumUseCase, mockTrackUseCase).Return(mockArtists, nil)
	...
	for idx, obj := range result.Result.([]interface{}) {
		...
		require.NoError(t, err)
		require.Equal(t, mockArtists[idx], *artist)
	}
}

func TestGetArtist(t *testing.T) {
	var mockArtist domain.ArtistDataTransfer
	err := faker.FakeData(&mockArtist)
	assert.NoError(t, err)

	e := echo.New()

	req, err := http.NewRequest(echo.GET, "/artists/"+strconv.Itoa(mockArtist.Id), strings.NewReader(""))
	assert.NoError(t, err)
	...
	c.SetParamNames("id")
	c.SetParamValues(strconv.Itoa(mockArtist.Id))

	mockTrackUseCase := new(mocks.TrackUseCaseInterface)
	mockAlbumUseCase := new(mocks.AlbumUseCaseInterface)
	mockArtistUseCase := new(mocks.ArtistUseCaseInterface)

	mockArtistUseCase.On("GetById", mockTrackUseCase, mockAlbumUseCase, mockArtist.Id).Return(&mockArtist, nil)
...
func TestUpdateArtist(t *testing.T) {
	...
	req, err := http.NewRequest(echo.PUT, "/artists/"+strconv.Itoa(mockArtist.Id), bytes.NewBuffer(dataToSend))
...
func TestCreateArtist(t *testing.T) {
	...
	dataToSend, err := json.Marshal(mockArtist)
	req, err := http.NewRequest(echo.POST, "/artists/"+strconv.Itoa(mockArtist.Id), bytes.NewBuffer(dataToSend))
	req.Header.Set(echo.HeaderContentType, echo.MIMEApplicationJSON)
	rec := httptest.NewRecorder()
	...
	expected := webUtils.Success{
		Status: webUtils.OK,
		Result: constants.SuccessCreated + "(" + fmt.Sprint(mockArtist.Id) + ")",
	}
...
func TestDeleteArtist(t *testing.T) {
	...
	req, err := http.NewRequest(echo.DELETE, "/artists/"+strconv.Itoa(mockArtist.Id), strings.NewReader(""))
	assert.NoError(t, err)
	...
	mockArtistUseCase := new(mocks.ArtistUseCaseInterface)
	mockArtistUseCase.On("Delete", mockArtist.Id).Return(nil)
	mockArtistUseCase.On("GetLastId").Return(mockArtist.Id, nil)
	handler := artistDeliveryHttp.Handler{
		ArtistUseCase: mockArtistUseCase,
		AlbumUseCase:  nil,
