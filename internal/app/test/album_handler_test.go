...
func TestGetEmptyAlbums(t *testing.T) {
	var mockAlbums = make([]domain.AlbumDataTransfer, 0)
	...
	mockAlbumUseCase.On("GetAll", mockTrackUseCase).Return(mockAlbums, nil)
...
func TestGetAllAlbums(t *testing.T) {
	var mockAlbums = make([]domain.AlbumDataTransfer, 10)
...
func TestGetAlbum(t *testing.T) {
	var mockAlbum domain.AlbumDataTransfer
	err := faker.FakeData(&mockAlbum)
	assert.NoError(t, err)

	e := echo.New()

	req, err := http.NewRequest(echo.GET, "/albums/"+strconv.Itoa(mockAlbum.Id), strings.NewReader(""))
	assert.NoError(t, err)
	...
	c.SetPath("/albums/:id")
	c.SetParamNames("id")
	c.SetParamValues(strconv.Itoa(mockAlbum.Id))

	mockTrackUseCase := new(mocks.TrackUseCaseInterface)

	mockAlbumUseCase := new(mocks.AlbumUseCaseInterface)
	mockAlbumUseCase.On("GetById", mockTrackUseCase, mockAlbum.Id).Return(&mockAlbum, nil)
...
func TestUpdateAlbum(t *testing.T) {
	...
	req, err := http.NewRequest(echo.PUT, "/albums/"+strconv.Itoa(mockAlbum.Id), bytes.NewBuffer(dataToSend))
...
func TestCreateAlbumError(t *testing.T) {
	...
	assert.Error(t, err)
	mockAlbum2 := domain.Album{}
	err = faker.FakeData(&mockAlbum2)
	mockAlbum2.Id = -50000

	dataToSend, err = json.Marshal(mockAlbum)
	req, err = http.NewRequest(echo.POST, "/albums/"+strconv.Itoa(mockAlbum2.Id), bytes.NewBuffer(dataToSend))
	req.Header.Set(echo.HeaderContentType, echo.MIMEApplicationJSON)
	rec = httptest.NewRecorder()
	c = e.NewContext(req, rec)
	c.SetPath("/albums/")
...
func TestCreateAlbum(t *testing.T) {
	...
	dataToSend, err := json.Marshal(mockAlbum)
	req, err := http.NewRequest(echo.POST, "/albums/"+strconv.Itoa(mockAlbum.Id), bytes.NewBuffer(dataToSend))
	req.Header.Set(echo.HeaderContentType, echo.MIMEApplicationJSON)
	rec := httptest.NewRecorder()
...
func TestDeleteAlbum(t *testing.T) {
	...
	req, err := http.NewRequest(echo.DELETE, "/albums/"+strconv.Itoa(mockAlbum.Id), strings.NewReader(""))
	assert.NoError(t, err)
